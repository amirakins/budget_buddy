...
class _TestPage2State extends State<TestPage2> {
  ...
  void initState() {
    ...
    // Set up a stream to listen for changes in the expenses collection
    expensesStream = FirebaseFirestore.instance.collection('expenses').snapshots();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: paymentAmount.isEmpty
        ...
        stream: expensesStream, // Use the expenses stream
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          // Clear and update expenses on every data change
          ...
          for (var category in expensesByCategory.keys) {
            for (var expense in expensesByCategory[category]!) {
              monthlyExpenses += expense['monthlyExpense'];
            }
          }
